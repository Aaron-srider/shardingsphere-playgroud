...
class CustomEncryptAlgorithm : StandardEncryptAlgorithm<Any?, String?> {
    ...
    override fun encrypt(plainValue: Any?, encryptContext: EncryptContext): String? {
        return try {
            if (null == plainValue) {
                ...
                val result =
                    getCipher(Cipher.ENCRYPT_MODE).doFinal(plainValue.toString().toByteArray(StandardCharsets.UTF_8))
